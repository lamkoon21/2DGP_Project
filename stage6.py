...
vengefly = []
save_bgm = None
change_bgm, play_bgm = False, False
boss_key = None
save_bench = None
...
def enter():
    ...
    save_bench = Savebench(1740, 560, 1910, 450)
    game_world.add_object(save_bench, 0)
    game_world.add_collision_pairs(server.knight, save_bench, 'knight:save_bench')
    
    if server.knight.boss_key < 3 and server.key_in_stage[2] == 0:
        global boss_key
        boss_key = Bosskey(3300, 2580)
    ...
    server.background.image = load_image('image/map/stage6.png')
...
def update():
    ...
    if server.current_stage == 5:
        import stage5
        game_framework.change_state(stage5)
    elif server.current_stage == 7:
        import stage7
    ...
    global change_bgm,play_bgm,save_bgm   
...
def set_knight(s):
    ...
    if server.current_stage == 'respawn2':
